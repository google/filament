...
This Go program consumes C++ header file(s) and generates JavaScript/TypeScript bindings and
C++ code that performs JSON serialization. (Java bindings generation for `View.java` and options
structs has been migrated to `tools/apigen`).
...
**codegen_java_flatten**    | Used by `apigen` to flatten nested sub-struct fields into the parent class.
**codegen_java_float**      | Used by `apigen` to force a `float` representation in Java.
...
Additionally, in-place edits are made to the following file:

- `web/filament-js/filament.d.ts`

*(Note: `View.java` and `View.cpp` bindings are now 100% generated by `tools/apigen`).*

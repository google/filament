...
// Everything after this marker is hand-written and is preserved across runs.
// A file without it behaves as before: the generated section runs to the end.
const kCodelineEndMarker = "End of beamsplitter-generated code"

// splitAroundMarkers reads a file that beamsplitter edits in place and returns
// the hand-written part before the generated section and the hand-written part
// after it. The trailing part is empty when the file carries no end marker.
func splitAroundMarkers(path string, what string) (head []string, tail []string) {
	...
	for lineScanner.Scan() {
		...
		switch {
		case !foundMarker && strings.Contains(codeline, kCodelineMarker):
			foundMarker = true
			inGenerated = true
		case inGenerated && strings.Contains(codeline, kCodelineEndMarker):
			inGenerated = false
		case inGenerated:
			// generated, will be replaced
		case foundMarker:
			tail = append(tail, codeline)
		default:
			head = append(head, codeline)
		}
	}
	if !foundMarker {
		log.Fatal("Unable to find marker line in " + what + " file.")
	}
	return head, tail
}
...
func EditTypeScript(definitions []db.TypeDefinition, namespace string, folder string) {
	...
	codelines, trailing := splitAroundMarkers(path, "TypeScript")
	...
	if len(trailing) > 0 {
		file.WriteString("// " + kCodelineEndMarker + "\n")
		for _, codeline := range trailing {
			file.WriteString(codeline)
			file.WriteString("\n")
		}
	}

...
vec3 surfaceShading(const PixelParams pixel, const Light light, float occlusion) {
    ...
    vec3 color = pixel.subsurfaceColor * (subsurface * Fd_Lambert());
    ...
    // Colored penumbra
    // At this stage, `color` holds the scattered light. We can use that term
    // directly as our penumbra color
    Fd = mix(color, Fd, pow4(occlusion));

    // NoL does not apply to transmitted light
    color += (Fd + Fr) * (NoL * occlusion);

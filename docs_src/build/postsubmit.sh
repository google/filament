...
function update_to_main() {
    ...
    git add docs/*
    git commit -A \
        -m "Updating /docs due to commit ${COMMIT_HASH:0:7}" \
        -m "" \
        -m "Full commit is ${COMMIT_HASH}" \
        -m "" \
        -m "DOCS_ALLOW_DIRECT_EDITS"

    # TODO: really enable it when ready
...
COMMIT_HASH=$1
HAS_EDITS=$((python3 docs_src/build/checks.py --do-or="source_edits,commit_docs_force" $COMMIT_HASH && echo "true") || echo "false")

...
Asset getCubemap(const char* name) {
    ...
    *shCoeffs = getRawFile(key.c_str());
    ...
    return {
        ...
        .envFaces = decltype(Asset::envFaces)(faces),
    };
}
...
SkyLight getSkyLight(Engine& engine, const char* name) {
    ...
    result.texture = Texture::Builder()
        .width(asset.width)
        .height(asset.height)
    ...
    for (uint32_t mip = 0, i = 0; mip < asset.envMipCount; ++mip, size >>= 1) {
        ...
        result.texture->setImage(engine, mip, std::move(buffer), offsets);
    }

    result.indirectLight = IndirectLight::Builder()
        .reflections(result.texture)
        .irradiance(3, result.bands)
        .intensity(30000.0f)
        .build(engine);

    result.skybox = Skybox::Builder().environment(result.texture).build(engine);

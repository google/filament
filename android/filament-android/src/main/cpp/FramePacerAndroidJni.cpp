...
extern "C" JNIEXPORT void JNICALL
Java_com_google_android_filament_android_FramePacer_nBuilderLatencyNanos(JNIEnv*, jclass, jlong nativeBuilder, jlong latencyNanos) {
    auto builder = (FramePacer::Builder*) nativeBuilder;
    builder->latency(std::chrono::nanoseconds(latencyNanos));
}
...
Java_com_google_android_filament_android_FramePacer_nSetupFrame(JNIEnv* env, jclass,
    ...
    return filament::android::wrapJni<jint>(env, [=]() {
        ...
        if (hardwareTimelines != nullptr && timelineCount > 0) {
            ...
            jint result = static_cast<jint>(pacer->setupFrame(tick));

            env->ReleaseLongArrayElements(hardwareTimelines, elements, JNI_ABORT);
            return result;
        }

        return static_cast<jint>(pacer->setupFrame(tick));
    });
}
...
extern "C" JNIEXPORT void JNICALL
Java_com_google_android_filament_android_FramePacer_nConfigure(JNIEnv* env, jclass, jlong nativeObject, jfloat targetFrameRate, jlong latencyNanos) {
    auto pacer = (FramePacer*) nativeObject;
    filament::android::wrapJni(env, [=]() {
        FramePacer::Configuration config;
        config.targetFrameRate = (float) targetFrameRate;
        config.latency         = std::chrono::nanoseconds(latencyNanos);

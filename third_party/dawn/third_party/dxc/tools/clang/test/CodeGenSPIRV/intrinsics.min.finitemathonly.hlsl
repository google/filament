// RUN: %dxc -ffinite-math-only -T vs_6_0 -E main -fcgl  %s -spirv | FileCheck %s
...
// CHECK: {{%[0-9]+}} = OpExtInst %float [[glsl]] FMin {{%[0-9]+}} {{%[0-9]+}}
  float a1,a2;
  result = min(a1,a2);
...
// CHECK: {{%[0-9]+}} = OpExtInst %v3float [[glsl]] FMin {{%[0-9]+}} {{%[0-9]+}}
  float3 c1,c2;
  result3 = min(c1,c2);
...
// CHECK: {{%[0-9]+}} = OpExtInst %v2float [[glsl]] FMin {{%[0-9]+}} {{%[0-9]+}}
  float1x2 e1,e2;
  result2 = min(e1,e2);

// CHECK: {{%[0-9]+}} = OpExtInst %v4float [[glsl]] FMin {{%[0-9]+}} {{%[0-9]+}}
  float4x1 f1,f2;
  result4 = min(f1,f2);
...
// CHECK-NEXT: [[result_row0:%[0-9]+]] = OpExtInst %v3float [[glsl]] FMin [[g1_row0]] [[g2_row0]]
// CHECK-NEXT: [[g1_row1:%[0-9]+]] = OpCompositeExtract %v3float [[g1]] 1
// CHECK-NEXT: [[g2_row1:%[0-9]+]] = OpCompositeExtract %v3float [[g2]] 1
// CHECK-NEXT: [[result_row1:%[0-9]+]] = OpExtInst %v3float [[glsl]] FMin [[g1_row1]] [[g2_row1]]

...
// CHECK-NEXT:      {{%[0-9]+}} = OpGroupNonUniformIAdd %v4int %uint_3 ExclusiveScan [[val1]]
    values[x].val1 = WavePrefixSum(val1);
// CHECK:      [[val2:%[0-9]+]] = OpLoad %v2uint %val2
// CHECK-NEXT:      {{%[0-9]+}} = OpGroupNonUniformIAdd %v2uint %uint_3 ExclusiveScan [[val2]]
    values[x].val2 = WavePrefixSum(val2);
// CHECK:      [[val3:%[0-9]+]] = OpLoad %float %val3
// CHECK-NEXT:      {{%[0-9]+}} = OpGroupNonUniformFAdd %float %uint_3 ExclusiveScan [[val3]]

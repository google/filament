...
uniform layout(binding = 0) highp sampler2D textureIn;
...
void main()
{
	vec2 sz = vec2(textureSize(textureIn, 0));

	ivec2 gid = ivec2(gl_GlobalInvocationID.xy);
	ivec2 lid = ivec2(gl_LocalInvocationID.xy);

	if (gid.x >= int(sz.x) || gid.y >= int(sz.y)) { return; }
	...
	if (lid.x < 5 && lid.y < 3)
	{
		...
		vec2 texCoords = (grpid + vec2(lid2)) / (sz);

		vec4 texGat = textureGather(textureIn, texCoords, 0);
	...
	int nNeighbours = (cache[lidy - 1][lidx - 1]) + (cache[lidy - 1][lidx]) + (cache[lidy - 1][lidx + 1]) + (cache[lidy][lidx - 1]) + (cache[lidy][lidx + 1]) +

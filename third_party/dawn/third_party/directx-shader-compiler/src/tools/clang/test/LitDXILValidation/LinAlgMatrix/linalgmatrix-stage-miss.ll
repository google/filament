...
; CHECK: Function: {{.*}}mainMS{{.*}}: error: Thread Group Shared Memory not supported from non-compute entry points.
...
; CHECK-NEXT: note: at {{.*}} @dx.op.linAlgMatrixLoadFromMemory
; CHECK-NEXT: Function: {{.*}}mainMS{{.*}}: error: Opcode LinAlgMatrixMultiply not valid in shader model lib_6_10(miss).
...
!18 = !{void (%struct.RayPayload*)* @"\01?mainMS@@YAXURayPayload@@@Z", !"\01?mainMS@@YAXURayPayload@@@Z", null, null, !19}
!19 = !{i32 8, i32 11, i32 6, i32 4, i32 5, !20}
!20 = !{i32 0}
!21 = !{!22}

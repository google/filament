...
// CHECK: [[a:%[a-zA-Z0-9_]+]] = OpImageSampleImplicitLod %v4float [[tex0]] [[v4fc]] None
  float4 a = tex.Sample(float4(0.5, 0.25, 0.75, 1.0));

// CHECK: [[tex1:%[a-zA-Z0-9_]+]] = OpLoad [[type_cube_array_sampled]] %tex
// CHECK: [[b:%[a-zA-Z0-9_]+]] = OpImageSampleImplicitLod %v4float [[tex1]] [[v4fc]] MinLod %float_0_5
  float4 b = tex.Sample(float4(0.5, 0.25, 0.75, 1.0), 0.5f);

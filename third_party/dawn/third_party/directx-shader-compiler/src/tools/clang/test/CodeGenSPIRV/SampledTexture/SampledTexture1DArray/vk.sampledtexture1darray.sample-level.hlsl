...
// CHECK: [[sampled_result:%[a-zA-Z0-9_]+]] = OpImageSampleExplicitLod %v4float [[tex1_load]] [[sample_coord:%[a-zA-Z0-9_]+]] Lod %float_0_5
// CHECK: OpStore %val1 [[sampled_result]]

    float4 val1 = tex1dArray.SampleLevel(float2(0.5, 0.25), 0.5f);

// CHECK: [[tex2_load:%[a-zA-Z0-9_]+]] = OpLoad %type_sampled_image %tex1dArray
// CHECK: [[sampled_result_2:%[a-zA-Z0-9_]+]] = OpImageSampleExplicitLod %v4float [[tex2_load]] [[sample_coord_2:%[a-zA-Z0-9_]+]] Lod|ConstOffset %float_0_5 %int_2
// CHECK: OpStore %val2 [[sampled_result_2]]

    float4 val2 = tex1dArray.SampleLevel(float2(0.5, 0.25), 0.5f, 2);

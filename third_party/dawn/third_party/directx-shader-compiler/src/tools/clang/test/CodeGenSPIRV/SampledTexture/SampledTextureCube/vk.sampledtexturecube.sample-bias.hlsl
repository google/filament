...
// CHECK: [[a:%[a-zA-Z0-9_]+]] = OpImageSampleImplicitLod %v4float [[tex0]] [[v3fc]] Bias %float_1
  float4 a = tex.SampleBias(float3(0.5, 0.25, 0.75), 1.0f);

// CHECK: [[tex1:%[a-zA-Z0-9_]+]] = OpLoad [[type_cube_sampled]] %tex
// CHECK: [[b:%[a-zA-Z0-9_]+]] = OpImageSampleImplicitLod %v4float [[tex1]] [[v3fc]] Bias|MinLod %float_1 %float_0_5
  float4 b = tex.SampleBias(float3(0.5, 0.25, 0.75), 1.0f, 0.5f);

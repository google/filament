...
// CHECK: [[sampled_result:%[a-zA-Z0-9_]+]] = OpImageSampleDrefImplicitLod %float [[tex1_load]] [[sample_coord:%[a-zA-Z0-9_]+]] %float_1 Bias %float_0_5
// CHECK: OpStore %val1 [[sampled_result]]

    float val1 = tex1dArray.SampleCmpBias(float2(0.5, 0.25), 1.0f, 0.5f);

// CHECK: [[tex2_load:%[a-zA-Z0-9_]+]] = OpLoad %type_sampled_image %tex1dArray
// CHECK: [[sampled_result_2:%[a-zA-Z0-9_]+]] = OpImageSampleDrefImplicitLod %float [[tex2_load]] [[sample_coord_2:%[a-zA-Z0-9_]+]] %float_1 Bias|ConstOffset %float_0_5 %int_2
// CHECK: OpStore %val2 [[sampled_result_2]]

    float val2 = tex1dArray.SampleCmpBias(float2(0.5, 0.25), 1.0f, 0.5f, 2);

// CHECK: [[tex3_load:%[a-zA-Z0-9_]+]] = OpLoad %type_sampled_image %tex1dArray
// CHECK: [[sampled_result_3:%[a-zA-Z0-9_]+]] = OpImageSampleDrefImplicitLod %float [[tex3_load]] [[sample_coord_3:%[a-zA-Z0-9_]+]] %float_1 Bias|ConstOffset|MinLod %float_0_5 %int_2 %float_2_5
// CHECK: OpStore %val3 [[sampled_result_3]]

    float val3 = tex1dArray.SampleCmpBias(float2(0.5, 0.25), 1.0f, 0.5f, 2, 2.5f);

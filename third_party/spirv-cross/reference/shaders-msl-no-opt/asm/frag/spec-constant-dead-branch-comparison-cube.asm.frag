...
fragment main0_out main0(main0_in in [[stage_in]], texturecube<float> ShadowMapCube [[texture(0)]], sampler Sampler [[sampler(0)]])
{
    main0_out out = {};
    if (get_sampler_type() == 3u)
    {
    }
    out.out_var_SV_Target0 = float4(ShadowMapCube.sample(Sampler, in.in_var_TEXCOORD0));
    return out;
}

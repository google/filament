...
function usage() {
  ...
  echo "  <version_tag>: The SPIRV-Tools version to download (e.g., 2023.4)"
  echo "  --sha <commit_sha>: The SPIRV-Tools commit SHA to download instead of a version tag."
  exit 1
}
...
if [[ "$1" == "--sha" ]]; then
    ...
    DOWNLOAD_REF=$2
    DOWNLOAD_URL="https://github.com/KhronosGroup/SPIRV-Tools/archive/${DOWNLOAD_REF}.zip"
    ...
    DOWNLOAD_REF=$1
    # The tags for SPIRV-Tools are like 'v2023.4'
    DOWNLOAD_URL="https://github.com/KhronosGroup/SPIRV-Tools/archive/refs/tags/v${DOWNLOAD_REF}.zip"
...
echo "Downloading SPIRV-Tools ${DOWNLOAD_REF}..."
if ! curl -f -L -o "${ZIP_FILE_NAME}" "${DOWNLOAD_URL}"; then
    echo "Error: Failed to download SPIRV-Tools ${DOWNLOAD_REF}." >&2
...
TEMP_DIR="spirv-tools_temp_unzip"
...
echo "Replacing existing spirv-tools with new version..."
# If spirv-tools_new exists, remove it.
rm -rf spirv-tools_new
mv "${EXTRACTED_DIR}" spirv-tools_new

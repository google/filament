...
if [[ "$os_name" == "Linux" ]]; then
    LINUX_BUILD_FLAG="-Dvulkan-drivers=swrast"
    ...
    fi # if [[ "$GITHUB_WORKFLOW" ]]
elif [[ "$os_name" == "Darwin" ]]; then
    brew install autoconf automake libx11 libxext libxrandr llvm ninja meson pipx
    pipx install mako
    ld -v    
    ld --version
fi
...
meson setup builddir/ -Dprefix="$(pwd)/out" -Dglx=xlib -Dosmesa=true -Dgallium-drivers=swrast ${LINUX_BUILD_FLAG}

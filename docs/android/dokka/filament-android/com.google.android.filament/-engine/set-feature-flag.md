//[filament-android](../../../index.md)/[com.google.android.filament](../index.md)/[Engine](index.md)/[setFeatureFlag](set-feature-flag.md)

# setFeatureFlag

[main]\
open fun [setFeatureFlag](set-feature-flag.md)(name: [String](https://developer.android.com/reference/kotlin/java/lang/String.html), value: Boolean): Boolean

Set the value of a non-constant feature flag.

#### Return

true if the value was set, false if the feature flag is constant or doesn't exist.

#### Parameters

main

| | |
|---|---|
| name | name of the feature flag to set |
| value | value to set |

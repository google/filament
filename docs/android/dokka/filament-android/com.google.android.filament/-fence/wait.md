...
[main]\
open fun [wait](wait.md)(mode: [Fence.Mode](-mode/index.md)): [Fence.FenceStatus](-fence-status/index.md)
...
|---|---|
| mode | Whether the command stream is flushed before waiting or not. |
| timeout | Wait time out in nanoseconds. Using a `timeout` of 0 is a way to query the state of the fence. A `timeout` value of FENCE_WAIT_FOR_EVER is used to disable the timeout. |

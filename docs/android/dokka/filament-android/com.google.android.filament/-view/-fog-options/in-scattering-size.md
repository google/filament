...
That is, the light from the sun that is scattered (by the fog) towards the camera. Size of the Sun in-scattering (>0 to activate). Good values are >>1 (e.g. ~10 - 100). Smaller values result is a larger scattering size. Ignored in `linearFog` mode.

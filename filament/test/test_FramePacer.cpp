...
/*
 * +------------------------------------+-----------------------------------------------------+---------------------------------------------------------+
 * | TEST SUITE NAME                    | SCENARIO                                            | EXPECTED BEHAVIOR                                       |
 * +------------------------------------+-----------------------------------------------------+---------------------------------------------------------+
 * | DefaultConfiguration               | Default build (60Hz Vsync, 2 latency frames)        | Approves Vsync, projects deadline base + 2 Vsync periods |
 * | TargetFrameRatePacing              | 30 FPS target requested on a 60Hz display           | Approves every 2nd Vsync, skips intermediate CPU cycles |
 * | TimelineMatching                   | Android API 33+ candidate hardware timelines        | Selects candidate timeline closest to target deadline   |
 * | DynamicDisplayRefreshRateChange    | Live dynamic 60Hz -> 120Hz display mode switch      | Automatically tracks active hardware period changes     |
 * | CustomLatencyFrames                | Single Buffering (1 frame) vs Triple Buffering (3)  | Scales presentation projections to match pipeline depth |
 * | ComplexHardwareCombinations        | Triple Buffering (3) at 30 FPS on a 120Hz display   | Accurately matches 30 FPS deadlines on 120Hz candidates |
 * | AnomalyRejectionRecovery           | Massive 500ms OS or CPU rendering thread stall      | Instantly rejects anomaly and re-anchors clock anchor   |
 * | RuntimeMidFlightReconfiguration    | App switches from 60 FPS (2 frames) to 30 FPS (1)   | Smoothly scales live runtime pacing step and depth      |
 * | NonIntegerRatioPacing              | 45 FPS target requested on 60Hz and 120Hz displays  | Preserves exact non-integer ratio (3 frames out of 4)   |
 * | FuzzyCadenceSynchronization        | 60.0 FPS or 30.0 FPS requested on 59.94Hz TV screen | Perfectly snaps target rate to active 59.94Hz cadence   |
 * | HardwareCapabilityCapping          | Excessive 90 FPS target requested on a 60Hz screen  | Clamps target pacing rate exactly to monitor maximum    |
 * | AsynchronousDisplayModeSwitch      | 120 FPS requested while screen is at 60Hz -> 120Hz  | Instantly re-anchors phase to new out-of-phase Vsync    |
 * | GenericPlatformNoTimelines         | Non-timelined targets (Desktop/iOS/WebGL/API 26-32) | Proves baseline, 30 FPS skip, and stalls without lines  |
 * | VsyncTelemetryJitterResilience     | Extreme Phase noise/jitter (+1.5ms, -1.2ms) on Vsync| Absorbs telemetry noise to maintain uniform projections |
 * | VariableRefreshRateContinuousSweep | Continuous VRR sweeps (120Hz -> 90 -> 60 -> 48Hz)   | Accurately updates active target steps and getters      |
 * | NonMonotonicBackwardsTimeJump      | Network time (NTP) steady clock jumps back by 500ms | Automatically catches time jump and recovers gracefully |
 * | RapidThermalThrottlingStressTest   | Continuous looper rapidly toggling 120/60/30 FPS    | 100% deterministic frame thread gating transitions      |
 * | ExtremeCloudGamingPipelineDepth    | Deep composite VR or Cloud streaming pipelines (8)  | Projects target presentation exact 8 frames in advance  |
 * | UltraLowFrameRateAnomalyRejection  | 5 FPS requested with a 150ms rendering thread stall | Dynamic anomaly window protects unshifted core phase   |
 * | GpuBoundSteadyState120Hz           | 120Hz display with a slow 25ms GPU rendering frame  | Locks into perfectly uniform 25ms presentation steady  |
 * | GpuBoundSteadyState60Hz            | 60Hz display with a slow 25ms GPU rendering frame   | Perfectly tracks alternating 16.6ms -> 33.3ms cadence  |
 * +------------------------------------+-----------------------------------------------------+---------------------------------------------------------+
 */
...
TEST_F(FramePacerTest, DefaultConfiguration) {
    ...
    EXPECT_TRUE(framePacer->setupFrame(tick));
...
TEST_F(FramePacerTest, TargetFrameRatePacing) {
    ...
    EXPECT_TRUE(framePacer->setupFrame(tick0));
    ...
    EXPECT_FALSE(framePacer->setupFrame(tick1)); // Correctly skipped!
    ...
    EXPECT_TRUE(framePacer->setupFrame(tick2)); // Approved!
...
TEST_F(FramePacerTest, DynamicDisplayRefreshRateChange) {
    ...
    EXPECT_TRUE(framePacer->setupFrame(tick60));
    ...
    EXPECT_TRUE(framePacer->setupFrame(tick120));

    // Expected presentation automatically tracks the new 120Hz period (8.33ms * 2)
    TimePoint expected120 = nextStart + (period120Hz * 2);
...
TEST_F(FramePacerTest, CustomLatencyFrames) {
    ...
    FramePacer::Configuration configSingle;
    configSingle.latencyFrames = 1;
    ...
    EXPECT_TRUE(pacerSingle->setupFrame(tickSingle));
    ...
    EXPECT_TRUE(pacerTriple->setupFrame(tickTriple));
...
TEST_F(FramePacerTest, AnomalyRejectionRecovery) {
    ...
    EXPECT_TRUE(framePacer->setupFrame(tickStall)); // Recovered and approved!
...
TEST_F(FramePacerTest, RuntimeMidFlightReconfiguration) {
    ...
    tick1.vsyncPeriod = period60Hz;
    EXPECT_TRUE(framePacer->setupFrame(tick1));
    TimePoint expected60 = start + (period60Hz * 2);
    ...
    tick2.vsyncPeriod = period60Hz;
    EXPECT_TRUE(framePacer->setupFrame(tick2));
...
TEST_F(FramePacerTest, NonIntegerRatioPacing) {
    ...
    tick0.baseTime = start;
    tick0.vsyncPeriod = period60Hz;
    EXPECT_TRUE(pacer60->setupFrame(tick0));
    ...
    tick1.baseTime = start + period60Hz;
    tick1.vsyncPeriod = period60Hz;
    EXPECT_TRUE(pacer60->setupFrame(tick1));
    ...
    tick2.baseTime = start + (period60Hz * 2);
    tick2.vsyncPeriod = period60Hz;
    EXPECT_FALSE(pacer60->setupFrame(tick2)); // Extraneous Choreographer call correctly skipped!
    ...
    EXPECT_TRUE(pacer60->setupFrame(tick3));
    ...
    EXPECT_TRUE(pacer120->setupFrame(tick120_0));
    ...
    EXPECT_TRUE(pacer120->setupFrame(tick120_3));
...
TEST_F(FramePacerTest, FuzzyCadenceSynchronization) {
    ...
    EXPECT_TRUE(pacer60->setupFrame(tick60));
    ...
    EXPECT_TRUE(pacer30->setupFrame(tick30));
    ...
    EXPECT_TRUE(pacer45->setupFrame(tick45));
...
TEST_F(FramePacerTest, HardwareCapabilityCapping) {
    ...
    EXPECT_TRUE(pacer90->setupFrame(tick90));
...
TEST_F(FramePacerTest, AsynchronousDisplayModeSwitch) {
    ...
    tick60.baseTime = start;
    tick60.vsyncPeriod = period60Hz;
    EXPECT_TRUE(pacer->setupFrame(tick60));
    ...
    EXPECT_TRUE(pacer->setupFrame(tick120)); // Must successfully approve and re-anchor!
...
TEST_F(FramePacerTest, GenericPlatformNoTimelines) {
    ...
    tick30_0.timelines   = {};
    EXPECT_TRUE(pacer30->setupFrame(tick30_0));
    ...
    tick30_1.timelines   = {};
    EXPECT_FALSE(pacer30->setupFrame(tick30_1));
    ...
    tick30_2.timelines   = {};
    EXPECT_TRUE(pacer30->setupFrame(tick30_2));
    ...
    EXPECT_TRUE(pacerStall->setupFrame(tickPreStall));
    ...
    EXPECT_TRUE(pacerStall->setupFrame(tickPostStall)); // Automatically recovers and re-anchors!
...
TEST_F(FramePacerTest, VsyncTelemetryJitterResilience) {
    ...
    for (size_t i = 0; i < jitterOffsets.size(); ++i) {
        ...
        tick.vsyncPeriod = period60Hz;
        EXPECT_TRUE(pacer->setupFrame(tick));
...
TEST_F(FramePacerTest, VariableRefreshRateContinuousSweep) {
    ...
    for (float freq : vrrFrequencies) {
        ...
        TimePoint expected = currentStart + (period * 2);
...
TEST_F(FramePacerTest, NonMonotonicBackwardsTimeJump) {
    ...
    tick0.baseTime    = start;
    tick0.vsyncPeriod = period60Hz;
    EXPECT_TRUE(pacer->setupFrame(tick0));
    ...
    EXPECT_TRUE(pacer->setupFrame(tickJump)); // Anomaly rejection must instantly approve and snap!
...
TEST_F(FramePacerTest, RapidThermalThrottlingStressTest) {
    ...
    EXPECT_TRUE(pacer->setupFrame(tick1));
...
TEST_F(FramePacerTest, UltraLowFrameRateAnomalyRejection) {
    ...
    EXPECT_TRUE(pacer->setupFrame(tickDelayed));
...
TEST_F(FramePacerTest, GpuBoundSteadyState120Hz) {
    ...
    for (int vsyncIndex = 0; vsyncIndex <= 15; ++vsyncIndex) {
        ...
        if (!gpuSim.shouldRenderFrame() || !pacer->setupFrame(tick)) {
            continue;
        }
...
TEST_F(FramePacerTest, MonotonicPresentationGuard) {
    ...
    // Frame 0 at T=0 with Latency 3 -> Approved (Expected presentation T=50ms)
    FramePacer::VsyncTick tick0;
    tick0.baseTime    = start;
    tick0.vsyncPeriod = period60Hz;
    EXPECT_TRUE(pacer->setupFrame(tick0));

    TimePoint expected0 = start + (period60Hz * 3);
    EXPECT_LT(std::chrono::abs(pacer->getExpectedPresentationTime() - expected0), std::chrono::nanoseconds(1000));

    // Reconfigure: User drops latencyFrames from 3 down to 2
    FramePacer::Configuration newConfig;
    newConfig.targetFrameRate = 60.0f;
    newConfig.latencyFrames   = 2;
    pacer->configure(newConfig);

    // Frame 1 at T=16.6ms with Latency 2 -> Would target T=50ms (same as Frame 0).
    // Monotonic Presentation Guard must skip it!
    FramePacer::VsyncTick tick1;
    tick1.baseTime    = start + period60Hz;
    tick1.vsyncPeriod = period60Hz;
    EXPECT_FALSE(pacer->setupFrame(tick1)); // Correctly skipped!

    // Frame 2 at T=33.3ms with Latency 2 -> Targets T=66.6ms (> 50ms). Approved!
    FramePacer::VsyncTick tick2;
    tick2.baseTime    = start + (period60Hz * 2);
    tick2.vsyncPeriod = period60Hz;
    EXPECT_TRUE(pacer->setupFrame(tick2));

    TimePoint expected2 = start + (period60Hz * 4);

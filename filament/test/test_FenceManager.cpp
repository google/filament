...
TEST_F(FenceManagerTest, TrackFrame) {
    ...
    EXPECT_FALSE(mFenceManager.hasCompleted(1));
}
...
TEST_F(FenceManagerTest, ReclaimWithNoFences) {
    ...
    EXPECT_FALSE(mFenceManager.isPending());
    // Serial 0 means "nothing submitted", which is trivially complete.
    EXPECT_TRUE(mFenceManager.hasCompleted(0));
}
...
TEST_F(FenceManagerTest, ReclaimWhenFenceSignaled) {
    ...
    mFenceManager.reclaimCompletedResources(mDriverApi);
    EXPECT_FALSE(mFenceManager.isPending());
    EXPECT_TRUE(mFenceManager.hasCompleted(1));
...
TEST_F(FenceManagerTest, ReclaimMultipleFencesPartial) {
    ...
    EXPECT_TRUE(mFenceManager.isPending());
    EXPECT_TRUE(mFenceManager.hasCompleted(1));
    EXPECT_TRUE(mFenceManager.hasCompleted(2));
    EXPECT_FALSE(mFenceManager.hasCompleted(3));

    EXPECT_CALL(mMockDriver, getFenceStatus(Handle<HwFence>(3)))
            .WillOnce(Return(FenceStatus::CONDITION_SATISFIED));
    mFenceManager.reclaimCompletedResources(mDriverApi);
    EXPECT_FALSE(mFenceManager.isPending());
    EXPECT_TRUE(mFenceManager.hasCompleted(3));
}

...
struct VulkanRenderTarget : private HwRenderTarget, fvkmemory::Resource {
    ...
    bool hasDepth() const { return bool(mDepth.texture); }
    bool isSwapChain() const { return !mOffscreen; }
    void bindToSwapChain(fvkmemory::resource_ptr<VulkanSwapChain> surf);

private:
    std::array<VulkanAttachment, MRT::MAX_SUPPORTED_RENDER_TARGET_COUNT> mColor = {};
    VulkanAttachment mDepth = {};
    std::array<VulkanAttachment, MRT::MAX_SUPPORTED_RENDER_TARGET_COUNT> mMsaaAttachments = {};
...
struct VulkanRenderPrimitive : public HwRenderPrimitive, fvkmemory::Resource {
    VulkanRenderPrimitive(PrimitiveType pt, Handle<HwVertexBuffer> vbh, Handle<HwIndexBuffer> ibh);

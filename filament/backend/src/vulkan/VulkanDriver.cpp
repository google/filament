...
void VulkanDriver::destroyVertexBufferInfo(Handle<HwVertexBufferInfo> vbih) {
    ...
    mResources.erase(vbi);
}

void VulkanDriver::createVertexBufferR(Handle<HwVertexBuffer> vbh,
        uint32_t vertexCount, Handle<HwVertexBufferInfo> vbih) {
    auto vertexBuffer = resource_ptr<VulkanVertexBuffer>::make(vbh,
            mContext, mStagePool, vertexCount, vbih);
    mResources.insert(vertexBuffer);
}
...
void VulkanDriver::destroyVertexBuffer(Handle<HwVertexBuffer> vbh) {
    ...
    auto vertexBuffer = resource_ptr<VulkanVertexBuffer>::cast(vbh);
    mResources.insert(vertexBuffer);
}
...
void VulkanDriver::createIndexBufferR(Handle<HwIndexBuffer> ibh, ElementType elementType,
    ...
    auto indexBuffer = resource_ptr<VulkanIndexBuffer>::make(ibh, mAllocator, mStagePool,
            elementSize, indexCount);
    mResources.insert(indexBuffer);
}
...
void VulkanDriver::destroyBufferObject(Handle<HwBufferObject> boh) {
    ...
    auto bufferObject = resource_ptr<VulkanBufferObject>::cast(boh);
    mResources.insert(bufferObject);
}
...
void VulkanDriver::createRenderTargetR(Handle<HwRenderTarget> rth,
    ...
    auto renderTarget = resource_ptr<VulkanRenderTarget>::make(rth, mPlatform->getDevice(),
    ...
    mResources.insert(renderTarget);
}
...
void VulkanDriver::destroyRenderTarget(Handle<HwRenderTarget> rth) {
    ...
    auto renderTarget = resource_ptr<VulkanRenderTarget>::cast(rth);
    if (UTILS_UNLIKELY(renderTarget == mDefaultRenderTarget)) {
        mDefaultRenderTarget = {};
    } else {
        mResources.erase(renderTarget);
    }
}

...
void VulkanDriver::updateDescriptorSetTexture(
    ...
    if (mExternalImageManager.isExternalImageTexture(texture)) {
        mExternalImageManager.bindExternalImage(set, binding, texture, params);
...
void VulkanDriver::createTextureExternalImage2R(Handle<HwTexture> th, backend::SamplerType target,
    ...
    mExternalImageManager.addExternalImageTexture(texture, externalImage);
...
void VulkanDriver::destroyTexture(Handle<HwTexture> th) {
    ...
    mExternalImageManager.removeExternalImageTexture(texture);
}

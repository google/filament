...
void CounterPoolImpl<THREAD_SAFE>::createCounter(CounterIndex freeInd, ResourceType type,
        HandleId id) {
    auto const impl = [&](typename CounterImpl<THREAD_SAFE>::DestructionFunc destroy) {
        Counter* ret = &mCounters[freeInd];
        new (ret) Counter(type, id, destroy);
    };
    if constexpr (THREAD_SAFE) {
        std::unique_lock<utils::Mutex> lock(mCountersMutex);
        impl(ResourceManager::destroyLaterWithType);
    } else {
        impl(ResourceManager::destroyWithType);
    }
}

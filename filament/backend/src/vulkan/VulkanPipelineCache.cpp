...
constexpr VkPrimitiveTopology getCanonicalPrimitiveTopology(VkPrimitiveTopology topology) noexcept {
    switch (topology) {
...
void setStencilOperation(VkCommandBuffer cmdbuffer, VkStencilFaceFlagBits faceMask, bool stencilWrite, StencilState::StencilOperations const& op) noexcept {
    vkCmdSetStencilOpEXT(cmdbuffer, faceMask, fvkutils::getStencilOp(op.stencilOpStencilFail),
            fvkutils::getStencilOp(op.stencilOpDepthStencilPass),
            fvkutils::getStencilOp(op.stencilOpDepthFail), fvkutils::getCompareOp(op.stencilFunc));
    vkCmdSetStencilCompareMask(cmdbuffer, faceMask, op.readMask);
    vkCmdSetStencilWriteMask(cmdbuffer, faceMask, stencilWrite ? op.writeMask : 0u);
    vkCmdSetStencilReference(cmdbuffer, faceMask, op.ref);
}
...
VkPipeline VulkanPipelineCache::createPipeline(
    ...
    // When `VK_DYNAMIC_STATE_PRIMITIVE_TOPOLOGY_EXT` is enabled, its only allowed
    // to change the state between the same topology class.
    VkPipelineInputAssemblyStateCreateInfo inputAssemblyState = {
        .sType = VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO,
        .topology = mHasDynamicState
                            ? getCanonicalPrimitiveTopology((VkPrimitiveTopology) key.topology)
                            : (VkPrimitiveTopology) key.topology,
    };
...
void VulkanPipelineCache::bindPrimitiveTopology(VkPrimitiveTopology topology) noexcept {
    if (mHasDynamicState) {
        ...
        mStaticPipelineKey.topology = getCanonicalPrimitiveTopology(topology);
    } else {
...
void VulkanPipelineCache::bindDynamicState(VkCommandBuffer cmdbuffer, uint16_t dirtyMask) {
    ...
    if (dirtyMask & DIRTY_STENCIL_STATE) {
        ...
        setStencilOperation(cmdbuffer, VK_STENCIL_FACE_FRONT_BIT, stencil.stencilWrite, stencil.front);
        setStencilOperation(cmdbuffer, VK_STENCIL_FACE_BACK_BIT, stencil.stencilWrite, stencil.back);
    }

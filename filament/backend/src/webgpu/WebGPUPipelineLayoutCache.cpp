...
void WebGPUPipelineLayoutCache::collectGarbage() {
    // remove any expired pipeline layouts...
    ...
    for (Iterator iterator{ mPipelineLayouts.begin() }; iterator != mPipelineLayouts.end();) {
        ...
        if (mFrameCount >
                (entry.lastUsedFrameCount + F_WEBGPU_PIPELINE_LAYOUT_EXPIRATION_IN_FRAME_COUNT)) {

...
WebGPUDriver::WebGPUDriver(WebGPUPlatform& platform) noexcept
    : mPlatform(platform) {
    if constexpr (FWGPU_ENABLED(FWGPU_PRINT_SYSTEM)) {
...
Handle<HwSwapChain> WebGPUDriver::createSwapChainS() noexcept {
    return Handle<HwSwapChain>((Handle<HwSwapChain>::HandleId) nextFakeHandle++);
}
...
Handle<HwTexture> WebGPUDriver::createTextureS() noexcept {
    return Handle<HwTexture>((Handle<HwTexture>::HandleId) nextFakeHandle++);
}
...
Handle<HwProgram> WebGPUDriver::createProgramS() noexcept {
    return Handle<HwProgram>((Handle<HwProgram>::HandleId) nextFakeHandle++);
}

Handle<HwFence> WebGPUDriver::createFenceS() noexcept {
    return Handle<HwFence>((Handle<HwFence>::HandleId) nextFakeHandle++);
}

Handle<HwTimerQuery> WebGPUDriver::createTimerQueryS() noexcept {
    return Handle<HwTimerQuery>((Handle<HwTimerQuery>::HandleId) nextFakeHandle++);
}

Handle<HwIndexBuffer> WebGPUDriver::createIndexBufferS() noexcept {
    return Handle<HwIndexBuffer>((Handle<HwIndexBuffer>::HandleId) nextFakeHandle++);
}
...
Handle<HwBufferObject> WebGPUDriver::createBufferObjectS() noexcept {
    return Handle<HwBufferObject>((Handle<HwBufferObject>::HandleId) nextFakeHandle++);
}

Handle<HwRenderTarget> WebGPUDriver::createRenderTargetS() noexcept {
    return Handle<HwRenderTarget>((Handle<HwRenderTarget>::HandleId) nextFakeHandle++);
}

Handle<HwVertexBuffer> WebGPUDriver::createVertexBufferS() noexcept {
    return Handle<HwVertexBuffer>((Handle<HwVertexBuffer>::HandleId) nextFakeHandle++);
}

Handle<HwDescriptorSet> WebGPUDriver::createDescriptorSetS() noexcept {
    return Handle<HwDescriptorSet>((Handle<HwDescriptorSet>::HandleId) nextFakeHandle++);
}

Handle<HwRenderPrimitive> WebGPUDriver::createRenderPrimitiveS() noexcept {
    return Handle<HwRenderPrimitive>((Handle<HwRenderPrimitive>::HandleId) nextFakeHandle++);
}

Handle<HwVertexBufferInfo> WebGPUDriver::createVertexBufferInfoS() noexcept {
    return Handle<HwVertexBufferInfo>((Handle<HwVertexBufferInfo>::HandleId) nextFakeHandle++);
}
...
Handle<HwDescriptorSetLayout> WebGPUDriver::createDescriptorSetLayoutS() noexcept {
    return Handle<HwDescriptorSetLayout>(
            (Handle<HwDescriptorSetLayout>::HandleId) nextFakeHandle++);
}
...
void WebGPUDriver::createSwapChainR(Handle<HwSwapChain> sch, void* nativeWindow, uint64_t flags) {
    WebGPUPlatform::SurfaceBundle surfaceBundle = mPlatform.createSurface(nativeWindow);
    mSurface = std::move(surfaceBundle.surface);
    // TODO we are ignoring surfaceBundle.fallbackExtent for now
    //      (which may or may not not contain anything useful for certain platforms).
    //      We should be storing this somewhere.
    mAdapter = mPlatform.requestAdapter(mSurface);
    if constexpr (bool(FWGPU_ENABLED(FWGPU_PRINT_SYSTEM))) {
        ...
        FWGPU_LOGI << "WebGPU surface fallback extent: width " << surfaceBundle.fallbackExtent.width
                   << " height " << surfaceBundle.fallbackExtent.height;
        if (surfaceBundle.fallbackExtent.width == 0 && surfaceBundle.fallbackExtent.height == 0) {
            FWGPU_LOGI << " (i.e. no fallback extent)";
        }
        FWGPU_LOGI << utils::io::endl;
    }
    ...
    // TODO configure the surface (maybe before or after creating the swapchain? how do we get the
    //                             surface extent?)
    ...
    if constexpr (!bool(FWGPU_ENABLED(FWGPU_PRINT_SYSTEM))) {
        FWGPU_LOGI << "If the FILAMENT_BACKEND_DEBUG_FLAG variable were set with the "
                   << utils::io::hex << FWGPU_PRINT_SYSTEM << utils::io::dec

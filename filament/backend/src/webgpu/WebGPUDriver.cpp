...
void WebGPUDriver::createSwapChainR(Handle<HwSwapChain> sch, void* nativeWindow, uint64_t flags) {
    WebGPUPlatform::SurfaceBundle surfaceBundle = mPlatform.createSurface(nativeWindow, flags);
    mSurface = std::move(surfaceBundle.surface);
    // TODO we are ignoring surfaceBundle.fallbackExtent for now
    //      (which may or may not not contain anything useful for certain
    //      platforms). We should be storing this somewhere.
...
void WebGPUDriver::setAcquiredImage(Handle<HwStream> sh, void* image,
        math::details::TMat33<float> const&, CallbackHandler* handler,

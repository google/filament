        ...
        for (int i = 0; i < MRT::MAX_SUPPORTED_RENDER_TARGET_COUNT; ++i) {
            if (colorInfos[i].handle) {
                ...
                if (colorTexture) {
                    ...
                    customColorMsaaSidecarViews[customColorViewCount] =
                            colorTexture->makeMsaaSidecarTextureViewIfApplicable(mipLevel,
                                    arrayLayer);
        ...
        if (depthStencilSourceHandle) {
            ...
            if (dsTexture) {
                ...
                customDepthStencilMsaaSidecarTextureView =
                        dsTexture->makeMsaaSidecarTextureViewIfApplicable(depthStencilMipLevel,
                                depthStencilArrayLayer);
...
void WebGPUDriver::resolve(Handle<HwTexture> destinationTextureHandle, const uint8_t sourceLevel,
    ...
    mMsaaTextureResolver.resolve(mCommandEncoder, *sourceTexture, sourceLevel, sourceLayer,
            *destinationTexture, destinationLevel, destinationLayer);
}

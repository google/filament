...
void WebGPURenderTarget::setUpRenderPassAttachments(wgpu::RenderPassDescriptor& outDescriptor,
    ...
    if (mHasDepthStencilAttachment) {
        ...
        if (depthStencilViewToUse) {
            ...
            if (hasDepth) {
                mDepthStencilAttachmentDesc.depthLoadOp = getLoadOperation(params, TargetBufferFlags::DEPTH);
                mDepthStencilAttachmentDesc.depthStoreOp = getStoreOperation(params, TargetBufferFlags::DEPTH);
                mDepthStencilAttachmentDesc.depthClearValue = params.clearDepth;
                mDepthStencilAttachmentDesc.depthReadOnly = (params.readOnlyDepthStencil & RenderPassParams::READONLY_DEPTH) > 0;
            } else {

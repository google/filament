...
void WebGPURenderTarget::setUpRenderPassAttachments(wgpu::RenderPassDescriptor& outDescriptor,
    ...
    if (mHasDepthStencilAttachment) {
        ...
        if (depthStencilViewToUse) {
            ...
            if (hasDepth) {
                mDepthStencilAttachmentDescriptor.depthLoadOp = getLoadOperation(params, TargetBufferFlags::DEPTH);
                mDepthStencilAttachmentDescriptor.depthStoreOp = getStoreOperation(params, TargetBufferFlags::DEPTH);

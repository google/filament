...
[[nodiscard]] wgpu::Buffer createBuffer(wgpu::Device const& device, const wgpu::BufferUsage usage,
    ...
    size += (F_WEBGPU_BUFFER_SIZE_MODULUS - (size % F_WEBGPU_BUFFER_SIZE_MODULUS)) %
            F_WEBGPU_BUFFER_SIZE_MODULUS;
...
void WebGPUBufferBase::updateGPUBuffer(BufferDescriptor const& bufferDescriptor,
    ...
    FILAMENT_CHECK_PRECONDITION(byteOffset % F_WEBGPU_BUFFER_SIZE_MODULUS == 0)
            << "Byte offset must be a multiple of " << F_WEBGPU_BUFFER_SIZE_MODULUS << " but is "
            << byteOffset;
    ...
    const size_t remainder = bufferDescriptor.size % F_WEBGPU_BUFFER_SIZE_MODULUS;
    ...
    if (remainder != 0) {
        ...
        // access this
        std::memset(mRemainderChunk.data() + remainder, 0, F_WEBGPU_BUFFER_SIZE_MODULUS - remainder);

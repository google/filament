...
WebGPUBufferBase::WebGPUBufferBase(wgpu::Device const& device, wgpu::BufferUsage usage,
        uint32_t size, char const* label)
...
void WebGPUBufferBase::updateGPUBuffer(BufferDescriptor const& bufferDescriptor,
    ...
    const auto legalSize = bufferDescriptor.size - remainder;

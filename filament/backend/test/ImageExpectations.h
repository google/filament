...
public:
    RenderTargetDump(filament::backend::DriverApi& api, filament::backend::RenderTargetHandle renderTarget, const ScreenshotParams& params);
...
public:
    ImageExpectation(const char* fileName, int lineNumber, filament::backend::DriverApi& api, ScreenshotParams params, filament::backend::RenderTargetHandle renderTarget);
...
private:
    ...
    int lineNumber_;
    ScreenshotParams params_;
    RenderTargetDump result_;
};
...
public:
     ...
     */
    void AddExpectation(const char* fileName, int lineNumber, filament::backend::RenderTargetHandle renderTarget, ScreenshotParams params);

    void Evaluate();

private:
    filament::backend::DriverApi& api_;
    std::vector<ImageExpectation> expectations_;
};

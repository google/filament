...
TEST_F(BlitTest, Blit2DTextureArray) {
    ...
    const int level = 0;
    Handle<HwRenderTarget> srcRenderTarget = mCleanup.add(api.createRenderTarget( TargetBufferFlags::COLOR,
            kSrcTexWidth >> level, kSrcTexHeight >> level, 1, 0, { srcTexture, level, kSrcTexLayer }, {}, {}));
    Handle<HwRenderTarget> dstRenderTarget = mCleanup.add(api.createRenderTarget( TargetBufferFlags::COLOR,

...
void ImageExpectation::compareImage() const {
    ...
    EXPECT_THAT(bytesFilled, testing::IsTrue()) << "Render target wasn't copied to the buffer for " << mFileName;
    if (bytesFilled) {
...
RenderTargetDump::RenderTargetDump(filament::backend::DriverApi& api, filament::backend::RenderTargetHandle renderTarget, const ScreenshotParams& params)
    ...
    filament::backend::PixelBufferDescriptor pb(mInternal->bytes.data(), size, filament::backend::PixelDataFormat::RGBA, filament::backend::PixelDataType::UBYTE, cb,
            (void*) mInternal.get());
    api.readPixels(renderTarget, 0, 0, mInternal->params.width(), mInternal->params.height(), std::move(pb));
#endif
}

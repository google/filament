    ...
    for (auto const& entry : passList) {
        ...
        auto& shadowPass = fg.addPass<ShadowPassData>("Shadow Pass",
                ...
                [=, &engine, &view](FrameGraphResources const& resources,
                    ...
                    // updatePrimitivesMorphTargets must be run after updatePrimitivesLod.
                    view.updatePrimitivesMorphTargets(engine, cameraInfo, scene->getRenderableData(), entry.range);

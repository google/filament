...
struct Material::BuilderDetails {
    ...
    std::map<
        CString,
        ExternalImageInfo> mExternalInfos;
    std::unordered_map<
        CString,
...
Material::Builder& Material::Builder::externalImageInfo(const char* UTILS_NONNULL name, uint64_t externalFormat,
        SamplerParams sampler, SamplerYcbcrConversion chroma) noexcept {
    FILAMENT_CHECK_PRECONDITION(name != nullptr) << "name cannot be null";
    mImpl->mExternalInfos[{name, std::strlen(name)}] = { static_cast<uint32_t>(externalFormat), sampler, chroma };
    return *this;
}
...
void FMaterial::processDescriptorSets(FEngine& engine, Builder const& builder, MaterialParser const* const parser) {
    UTILS_UNUSED_IN_RELEASE bool success;

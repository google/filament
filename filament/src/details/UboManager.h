...
public:
    ...
    public:
        ...
        [[nodiscard]] bool isPending() const noexcept {
            return mSubmittedSerial > mCompletedSerial;
        }

        [[nodiscard]] bool hasCompleted(Serial serial) const noexcept {
...
private:
...
#ifndef NDEBUG
    // True if every allocator slot is owned by exactly one of: a managed instance,
    // mFreedAllocations, or mRetiredAllocations. Used by endFrame() to justify skipping a fence.
    [[nodiscard]] bool isAllocationAccountingConsistent() const noexcept;
#endif
    ...
    // INVARIANT: sorted by non-decreasing `serial`. beginFrame() relies on this to retire a
    // prefix and erase it in one pass. It holds because entries are only appended by
    // deferRetirement(), which stamps them with the monotonic submitted serial, and because
    // this list is cleared whenever FenceManager::reset() restarts the serial counter.

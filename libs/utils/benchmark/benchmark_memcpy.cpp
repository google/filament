...
struct AlignedAllocator {
    ...
    T* allocate(size_t const n) {
        ...
        // Note: On Android (Bionic libc), this requires API Level 28+ (Android 9+)
        if (__builtin_available(android 28, *)) {
            void* ptr = std::aligned_alloc(Align, aligned_bytes);
...
template<typename T>
using PageAlignedVector = std::vector<T, AlignedAllocator<char, ALIGNMENT>>;

...
Filament.loadClassExtensions = function() {
    ...
    Filament.Engine.create = function (canvas, options, config) {
        ...
        // The web build reaches three kinds of device: NOOP, which needs none at all and is
        // what lets the tests run under Node with no canvas, WebGPU, which creates its own, and
        // GL, which takes the context emscripten registers. DEFAULT resolves to GL.
        if (backend === Filament.Backend.VULKAN || backend === Filament.Backend.METAL) {
            throw new Error('The web build has no VULKAN or METAL backend.');
        }

        // Only GL consumes the emscripten-registered context; leaving the transient globals
        // unset is what tells Engine$Builder._build to skip registration.
